...
const BrowserPage = route => {
  ...
  }, "Settings"))), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(_chakra_ui_core__WEBPACK_IMPORTED_MODULE_2__["Box"], {
    position: "absolute",
    bg: "#f6f6f7",
    w: "100%",
    h: "90%",
...
const Wallets = () => {
  ...
  const generateWallet = async () => {
    ...
    const blob = new Blob([wallet.key], {
      type: 'application/json'
    });
    const href = await URL.createObjectURL(blob);
    const link = document.createElement('a');
  ...
  const loadWallet = async (nickname, password) => {
    ...
    Object(_providers_wallets__WEBPACK_IMPORTED_MODULE_2__["addWallet"])(wallet, nickname, password).then(res => {
      console.log(res);
      toast({
